//! Activation approximations from RNNoise used by WebRTC RNN-VAD.
...
pub fn tansig_approximated(mut x: f32) -> f32 {
    ...
    let mut y = TANSIG_TABLE[i];
    x -= 0.04 * i as f32;
    y = y + x * (1.0 - y * y) * (1.0 - y * x);
    sign * y
}
...
mod tests {
    ...
    #[test]
    fn tansig_is_bounded_and_odd() {
        let xs = [-10.0, -8.0, -4.0, -1.2, -0.1, 0.0, 0.1, 1.2, 4.0, 8.0, 10.0];
        for &x in &xs {
            let y = tansig_approximated(x);
            assert!((-1.0..=1.0).contains(&y));
            let yo = tansig_approximated(-x);
            assert!((y + yo).abs() < 1e-4, "x={x}, y={y}, yo={yo}");
        }
    }

    #[test]
    fn sigmoid_matches_tansig_relation() {
        let xs = [-10.0, -3.0, -0.5, 0.0, 0.5, 3.0, 10.0];
        for &x in &xs {
            let s = sigmoid_approximated(x);
            let relation = 0.5 + 0.5 * tansig_approximated(0.5 * x);
            assert!((s - relation).abs() < 1e-6);
    ...
    fn relu_clamps_negative() {
        assert_eq!(0.0, rectified_linear_unit(-2.0));
        assert_eq!(0.0, rectified_linear_unit(0.0));
        assert_eq!(3.5, rectified_linear_unit(3.5));
    }
}
